...
public:
    ...
    void Li_first_path(const RayDifferential &r, RadianceQueryRecord &rRec, 
        ...
        std::vector<Float> &pathBSDFPdfs,
        std::vector<Float> &pathNEEPdfs,
        std::vector<Spectrum> &neePathThroughputs,
        std::vector<Float> &neePathBSDFPdfs,
        std::vector<Float> &neePathNEEPdfs
        ) const {
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
                ...
                if (!value.isZero()) {
                    ...
                    if (!bsdfVal.isZero() && (!m_strictNormals
                        ...
                        if(emitter->isOnSurface()){
                            ...
                            neePathNEEPdfs.push_back(path_pdf * dRec.pdf * G);
                            neePathBSDFPdfs.push_back(path_pdf * bsdfPdf * G);
                            neePathThroughputs.push_back(path_throughput * bsdfVal * G * dRec.pdf * value * em_modulation_weight);
                        } else {
                            neePathNEEPdfs.push_back(dRec.pdf * path_pdf);
                            neePathBSDFPdfs.push_back(0);
                            neePathThroughputs.push_back(path_throughput * bsdfVal * dRec.pdf * value * em_modulation_weight);
                        }
                        neeAdded = true;
                    }
                }
            ...
            if(!neeAdded) {
                neePathNEEPdfs.push_back(0);
                neePathBSDFPdfs.push_back(0);
                neePathThroughputs.push_back(Spectrum(0.0f));
            }
            ...
            if (hitEmitter &&
                ...
                if(!(bRec.sampledType & BSDF::EDelta)){
                    pathNEEPdfs.push_back(path_pdf * lumPdf * G);
                    pathBSDFPdfs.push_back(path_pdf * bsdfPdf * G);
                    pathThroughputs.push_back(path_throughput * bsdfWeight * bsdfPdf * value * G * modulation_weight);
                } else {
                    pathNEEPdfs.push_back(0);
                    pathBSDFPdfs.push_back(path_pdf);
                    pathThroughputs.push_back(path_throughput * bsdfWeight * bsdfPdf * value * modulation_weight);
                }
                // Li += throughput * value * miWeight(bsdfPdf, lumPdf) * modulation_weight;
            } else {
                pathNEEPdfs.push_back(0);
                pathBSDFPdfs.push_back(0);
                pathThroughputs.push_back(Spectrum(0.0f));
            }

            if(!(bRec.sampledType & BSDF::EDelta)){
                path_pdf *= bsdfPdf * G;
                path_throughput *= bsdfWeight * bsdfPdf * G;
            } else {
                path_pdf *= bsdfPdf;
                path_throughput *= bsdfWeight * bsdfPdf;
            }
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const
    {
        ...
        std::vector<Float> pathBSDFPdfs2;
        std::vector<Float> pathNEEPdfs2;
        std::vector<Spectrum> neePathThroughputs2;
        std::vector<Float> neePathBSDFPdfs2;
        std::vector<Float> neePathNEEPdfs2;
        ...
        Li_first_path(r, rRec, path, neePath, pathThroughputs, pathBSDFPdfs, pathNEEPdfs, neePathThroughputs, neePathBSDFPdfs, neePathNEEPdfs);
        int path_n = neePathThroughputs.size();
        ...
        Li_second_path(r2, rRec2, path, neePath, pathThroughputs2, pathBSDFPdfs2, pathNEEPdfs2, neePathThroughputs2, neePathBSDFPdfs2, neePathNEEPdfs2);

        Spectrum Li(0.0f);
        for(int i=0; i<neePathThroughputs.size(); i++){
            Spectrum f_nee = neePathThroughputs.at(i);
            Float p_nee_bsdf = neePathBSDFPdfs.at(i);
            Float p_nee_nee = neePathNEEPdfs.at(i);
            if(p_nee_nee > 0){
                Li += f_nee / p_nee_nee * miWeight(p_nee_nee, p_nee_bsdf);
            }
        }
        for(int i=0; i<pathThroughputs.size(); i++){
            Spectrum f_bsdf = pathThroughputs.at(i);
            Float p_bsdf_bsdf = pathBSDFPdfs.at(i);
            Float p_bsdf_nee = pathNEEPdfs.at(i);
            if(p_bsdf_bsdf > 0){    
                Li += f_bsdf / p_bsdf_bsdf * miWeight(p_bsdf_bsdf, p_bsdf_nee);
            }
        }
