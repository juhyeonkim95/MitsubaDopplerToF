    ...
    Spectrum Li_helper(const RayDifferential &r, RadianceQueryRecord &rRec, Float ray2_time) const {
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            ...
            if(strcmp(m_antithetic_sampling_mode.c_str(), "mis")==0){
                ...
                Float mis_position = reflected_dist * reflected_dist;//miWeight(path_pdf_2_as_1, path_pdf_3_as_1);
                mis_position = std::min(mis_position, 1.0);
                Float mis_direction = 1 - mis_position;//miWeight(path_pdf_3_as_1, path_pdf_2_as_1);
                ...
                Li += 0.5f * mis_direction * neePathThroughput1 / path_pdf_1_as_1;//* miWeight3(path_pdf_1_as_1, path_pdf_1_as_3, 0);
            ...
            n_dot_v = std::abs(dot(ray.d, its.shFrame.n));
            Vector3 reflected_dir = reflect(-ray.d, its.shFrame.n);
            Ray ray_temp = Ray(its.p, reflected_dir, ray.time);
            Intersection its_temp;
            if(scene->rayIntersect(ray_temp, its_temp)){
                reflected_dist = its_temp.t;
            } else {
                reflected_dist = -1;
            }
