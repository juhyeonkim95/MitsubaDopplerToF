...
public:
    ...
    Spectrum accumulate_nee_Li(PathTracePart &path1, PathTracePart &path2, bool debug=false) const{
        if(path1.path_pdf_nee_as_nee > 0){
            ...
            // if( path1.prev_its.shFrame.n.y == 1.0 && 
            //     (path2.its.p - path1.its.p).lengthSquared() > 1e-6 && debug){
                
            //     if(std::abs(mis_primal_antithetic - 0.5) > 1e-2 && std::abs(mis_primal_antithetic - 1.0) > 1e-2){
            //         printf("%f\n", mis_primal_antithetic);
            //     }
                
            //     // printPoint(path1.its.p, path2.its.p);
            //     // printf("Throughput %f, %f\n", path1.path_throughput_nee[0], path2.path_throughput_nee[0]);
            //     // printf("G %f, %f\n", path1.G, path2.G);
            //     // printf("neeG %f, %f\n", path1.G_nee, path2.G_nee);
            // }
    ...
    Spectrum Li_helper(const RayDifferential &r, RadianceQueryRecord &rRec, Float ray2_timeSample) const {
        ...
        while(rRec.depth <= m_maxDepth || m_maxDepth < 0){
            ...
            // Accumulate nee Li
            Li += accumulate_nee_Li(path1, path2, true);
            Li += accumulate_nee_Li(path2, path1, true);
            ...
            // if(path1.prev_its.shFrame.n.y == 1.0 && (path2.its.p - path1.its.p).lengthSquared() > 1e-6 && path2.G > 1e6){
            //     printPoint(path1.its.p, path2.its.p);
            //     printf("Throughput %f, %f\n", path1.path_throughput[0], path2.path_throughput[0]);
            //     printf("G %f, %f\n", path1.G, path2.G);
            // }
            ...
            // if(rRec.depth == 1 && path1.prev_its.shFrame.n.y == 1.0 && 
            //     (path2.its.p - path1.its.p).lengthSquared() > 1e-6 && path1.G > 10000){
            //         return Spectrum(0.0);  
            //     if(path1.path_pdf_as(path1) > 10 * path1.path_pdf_as(path2)){
            //         return Spectrum(0.0);
            //         printPoint(path1.its.p, path2.its.p);
            //         printf("Throughput %f, %f\n", path1.path_throughput[0], path2.path_throughput[0]);
            //         printf("G %f, %f\n", path1.G, path2.G);
            //         printf("Pdf %f, %f\n", path1.path_pdf_as(path1), path1.path_pdf_as(path2));
            //     }
            // }
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const
    {
        ...
        if(strcmp(m_antithetic_sampling_mode.c_str(), "mis")==0){
            return Li_helper_mis_new(r, rRec, ray2_time);
        } 
