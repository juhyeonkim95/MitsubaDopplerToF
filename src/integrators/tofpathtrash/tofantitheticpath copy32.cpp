...
public:
    ToFAntitheticPathTracer(const Properties &props)
        ...
        std::cout << "WAVE FUNCTION TYPE: " << wave_function_type_str << std::endl;
    ...
    Spectrum Li_helper(
        ...
        while(rRec.depth <= m_maxDepth || m_maxDepth < 0){
            ...
            if(path1.its.isValid()){
                ...
                its_roughness = std::min(its_roughness, 5.0);
                ...
                if(strcmp(m_spatial_correlation_mode.c_str(), "adaptive") == 0 && rRec.depth == 1){
