...
public:
    ...
    Spectrum Li_helper(const RayDifferential &r, RadianceQueryRecord &rRec, Float ray2_time) const {
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            ...
            Vector3 reflected_dir2 = reflect(-ray2.d, its2.shFrame.n);
            Ray ray2_mirror = Ray(its2.p, reflected_dir2, ray2.time);
            ...
            if(!path2_blocked && useMixing){
                ...
                if(next_its_p.instance){
                    const Instance *instance1 = reinterpret_cast<const Instance*>(its.instance);
                    const Instance *instance2 = reinterpret_cast<const Instance*>(next_its_p.instance);

                    Transform trafo1 = instance1->getAnimatedTransform()->eval(ray.time);
                    Transform trafo2 = instance2->getAnimatedTransform()->eval(ray2.time);
                    
                    Ray ray_mirror_local;
                    Ray ray2_mirror_local;
                    Ray ray2_d_local;
                    Ray ray_local;

                    trafo1.inverse()(ray_mirror, ray_mirror_local);
                    trafo1.inverse()(ray, ray_local);
                    trafo2.inverse()(ray2_mirror, ray2_mirror_local);
                    trafo2.inverse()(ray2_d, ray2_d_local);
                    ...
                    if (shape_name.find("Rectangle") != std::string::npos){
                        ...
                        //next_its_d.p = trafo2(ray_temp(t_temp));
                        //t_area = 1.0;
                        //t_dist = 1.0;
                        //t_cos = 1.0;
                    } else {
                        // Triangle::rayIntersectForced(its.p0, its.p1, its.p2, ray2_d_local, temp[0], temp[1], t_temp);
                        
                        Float u_mirror, v_mirror, t_mirror;
                        Float u_mirror2, v_mirror2, t_mirror2;
                        bool ray_mirror_hit = Triangle::rayIntersectForced(its.p0, its.p1, its.p2, ray_mirror_local, u_mirror, v_mirror, t_mirror);
                        bool ray2_mirror_hit = Triangle::rayIntersectForced(its.p0, its.p1, its.p2, ray2_mirror_local, u_mirror2, v_mirror2, t_mirror2);

                        // bool ray_mirror_hit = (u_mirror > 0) && (u_mirror < 1) && (v_mirror > 0) && (v_mirror < 1) 
                        //             && (1-u_mirror-v_mirror > 0) && (1-u_mirror-v_mirror < 1);
                        // bool ray2_mirror_hit = (u_mirror2 > 0) && (u_mirror2 < 1) && (v_mirror2 > 0) && (v_mirror2 < 1) 
                        //             && (1-u_mirror2-v_mirror2 > 0) && (1-u_mirror2-v_mirror2 < 1);

                        Point p0 = trafo1(its.p0);
                        Point p1 = trafo1(its.p1);
                        Point p2 = trafo1(its.p2);
                        Point pa0 = trafo2(its.p0);
                        Point pa1 = trafo2(its.p1);
                        Point pa2 = trafo2(its.p2);

                        Point ray_mirror_hit_p = trafo1(ray_mirror_local(t_mirror));
                        Point ray2_mirror_hit_p = trafo2(ray2_mirror_local(t_mirror2));

                        Float u_ray, v_ray, t_ray;
                        Point ph0, ph1, ph2;
                        Point pha0, pha1, pha2;
                        //ray_mirror_hit && ray2_mirror_hit
                        if(ray_mirror_hit && ray2_mirror_hit){
                            bool ray_mirror_hit_in_tri_1 = Triangle::rayIntersect(p0, p1, ray_mirror_hit_p, ray, u_ray, v_ray, t_ray);
                            if(ray_mirror_hit_in_tri_1){
                                ph0 = p0;
                                ph1 = p1;
                                ph2 = ray_mirror_hit_p;
                                pha0 = pa0;
                                pha1 = pa1;
                                pha2 = ray2_mirror_hit_p;
                            } else {
                            bool ray_mirror_hit_in_tri_2 = Triangle::rayIntersect(ray_mirror_hit_p, p1, p2, ray, u_ray, v_ray, t_ray);
                            if(ray_mirror_hit_in_tri_2){
                                ph0 = ray_mirror_hit_p;
                                ph1 = p1;
                                ph2 = p2;
                                pha0 = ray2_mirror_hit_p;
                                pha1 = pa1;
                                pha2 = pa2;
                            }
                            else{
                                Triangle::rayIntersect(p0, ray_mirror_hit_p, p2, ray, u_ray, v_ray, t_ray);
                                ph0 = p0;
                                ph1 = ray_mirror_hit_p;
                                ph2 = p2;
                                pha0 = pa0;
                                pha1 = ray2_mirror_hit_p;
                                pha2 = pa2;
                            }
                            }
                            //bool ray_mirror_hit_in_tri_3 = Triangle::rayIntersect(p0, ray_mirror_hit_p, p2, ray, u_mirror, v_mirror, t_mirror);
                        } else {
                            Triangle::rayIntersect(p0, p1, p2, ray, u_ray, v_ray, t_ray);
                        ...
                        //Point pppp = its.barycentric[0] * its.p0 + its.barycentric[1] * its.p1 + its.barycentric[2] * its.p2;
                        //pppp = trafo1(pppp);
                        //Point ppp2 = ray.o + its.t * ray.d;
                        //printf("A: %f, %f, %f / %f, %f, %f\n", pppp.x, pppp.y, pppp.z, its.p.x, its.p.y, its.p.z);
                        //printf("B:%f, %f, %f / %f, %f, %f\n", ppp2.x, ppp2.y, ppp2.z, its.p.x, its.p.y, its.p.z);
                        
                        //Triangle::rayIntersect(its.p0, its.p1, its.p2, ray_local, u_ray, v_ray, t_ray);
                        //printf("A : %f, %f, %f / %f, %f, %f\n", u_ray, v_ray, t_ray, its.barycentric[1], its.barycentric[2], its.t);
                        //Triangle::rayIntersect(p0, p1, p2, ray, u_ray, v_ray, t_ray);
                        //printf("B : %f, %f, %f / %f, %f, %f\n", u_ray, v_ray, t_ray, its.barycentric[1], its.barycentric[2], its.t);
                        
                        next_its_m.p = pha0 * (1 - u_ray - v_ray) + pha1 * u_ray + pha2 * v_ray;
                            
                        Vector eh1 = ph1 - ph0;
                        Vector eh2 = ph2 - ph0;
                        Float area_1 = cross(eh1, eh2).length();
                        Vector eha1 = pha1 - pha0;
                        Vector eha2 = pha2 - pha0;
                        Float area_2 = cross(eha1, eha2).length();
                        
                        jacobian_m = area_2 / area_1;
                        //std::cout << "Area: " << jacobian_m << std::endl;

                        //jacobian_m = 1.0;
                        //std::cout << next_its_m.p.toString() << next_its_p.p.toString() << std::endl;
                        //printf("%f, %f / %f, %f\n", u_ray, v_ray, its.barycentric[1], its.barycentric[2]);
