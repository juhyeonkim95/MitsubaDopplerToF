...
public:
    ...
    Spectrum accumulate_nee_Li(PathTracePart &path1, PathTracePart &path2, bool debug=false) const{
        if(path1.path_pdf_nee_as_nee > 0){
            ...
            // Float mis_primal_antithetic = miWeight(p11, p12, 0.0);
    ...
    std::pair<Spectrum, Spectrum> precalculate_positional_directional_error_helper(
        ...
        while(rRec.depth <= m_maxDepth || m_maxDepth < 0){
            ...
            if (!path3.path_terminated && ((rRec3.depth >= m_maxDepth && m_maxDepth > 0)
                || (m_strictNormals && dot(path3.ray.d, path3.its.geoFrame.n)
                    * Frame::cosTheta(path3.its.wi) >= 0))) {
            ...
            else if(!path1.path_terminated && path3.path_terminated){
            ...
            // direct emission
            if(rRec.depth == 1 && !m_hideEmitters){
                //Li12 += accumulate_next_ray_Li(path1, path2);
                //Li12 += accumulate_next_ray_Li(path2, path1);
                //Li13 += accumulate_next_ray_Li(path1, path3);
                //Li13 += accumulate_next_ray_Li(path3, path1);
            }
            ...
            // Accumulate nee Li
            if(rRec.depth > 1){
                Li12 += accumulate_nee_Li(path1, path2);
                Li12 += accumulate_nee_Li(path2, path1);

                Li13 += accumulate_nee_Li(path1, path3);
                Li13 += accumulate_nee_Li(path3, path1);
            }
            ...
            // Li += accumulate_next_ray_Li(path1, path3);
            // Li += accumulate_next_ray_Li(path3, path1);
        ...
        return {Li12, Li13};//std::make_pair<Spectrum, Spectrum>();
    }
    ...
    std::pair<Spectrum, Spectrum> precalculate_positional_directional_error(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        // 1. Time sampling
        ...
        return precalculate_positional_directional_error_helper(r1, r2, rRec);
    }
